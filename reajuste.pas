...
// fim da estrutura 1 - condicional por cargo
    ...
    If (tempo_empresa = 1) and ( tempo_empresa < 10 ) and (codigo_cargo = 1) then
           vl_acrescimo := salario_atual * (1.5 / 100)
        Else

    If (tempo_empresa = 1) and (tempo_empresa < 10 ) and (codigo_cargo = 2) then
           vl_acrescimo := salario_atual * (2.0 / 100)
        Else

    If (tempo_empresa = 1) and (tempo_empresa < 10 ) and (codigo_cargo = 3) then
...
// fim dos 3 If para 1 a 10 anos
    ...
    If (tempo_empresa = 10) and (tempo_empresa < 20 ) and (codigo_cargo = 1) then
           vl_acrescimo := salario_atual * (2.0 / 100)
        Else

    If (tempo_empresa = 10) and (tempo_empresa < 20 ) and (codigo_cargo = 2) then
           vl_acrescimo := salario_atual * (2.5 / 100)
        Else

    If (tempo_empresa = 10) and (tempo_empresa < 20 ) and (codigo_cargo = 3) then
...
// fim dos 3 If para 10 a 20 anos
   ...
   If (tempo_empresa > 20 ) and (codigo_cargo = 1) then
    ...
    If (codigo_cargo = 1 ) then
        ...
        descr_cargo := ('Auxiliar Administrativo');

    If (codigo_cargo = 2 ) then
        descr_cargo := ('T‚cnico de Hardware');

    If (codigo_cargo = 3 ) then
        descr_cargo := ('Encarregado');
 ...
 // mostrar tudo o que foi coletado e calculado
    ...
    Writeln;
    Writeln ('Cargo: ',codigo_cargo, ' - ', descr_cargo);
    Writeln;
